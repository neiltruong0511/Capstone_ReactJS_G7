...
const EditFilmPage = () => {
  ...
  const { data, isLoading } = useQuery(["movieDetail", idFilm], async () => {
    ...
    <div className="space-y-6 text-slate-100">
      ...
      <div className="rounded-3xl border border-slate-800 bg-slate-950/70 p-6 shadow-2xl shadow-slate-950/40">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-amber-400">Chỉnh sửa phim</p>
            <h1 className="mt-2 text-3xl font-semibold text-white">{data?.tenPhim || "Phim mới"}</h1>
          </div>
      ...
      <form onSubmit={formik.handleSubmit} className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-4 rounded-3xl border border-slate-800 bg-slate-950/70 p-6 shadow-lg shadow-slate-950/20">
          ...
          <div className="h-72 overflow-hidden rounded-3xl border border-slate-800 bg-slate-900">
            {imagePreview ? (
              ...
              <div className="flex h-full items-center justify-center text-sm text-slate-500">Chưa có ảnh</div>
            )}
          </div>
        ...
        <div className="lg:col-span-2 space-y-4 rounded-3xl border border-slate-800 bg-slate-950/70 p-6 shadow-lg shadow-slate-950/20">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Tên phim</label>
              <input type="text" {...formik.getFieldProps("tenPhim")} className="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2.5 text-white outline-none focus:border-amber-400" />
              {formik.touched.tenPhim && formik.errors.tenPhim && <p className="mt-1 text-xs text-rose-400">{formik.errors.tenPhim}</p>}
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Bí danh</label>
              <input type="text" {...formik.getFieldProps("biDanh")} className="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2.5 text-white outline-none focus:border-amber-400" />
              {formik.touched.biDanh && formik.errors.biDanh && <p className="mt-1 text-xs text-rose-400">{formik.errors.biDanh}</p>}
            </div>
          </div>

          <div>
            <label className="block text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Trailer</label>
            <input type="text" {...formik.getFieldProps("trailer")} className="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2.5 text-white outline-none focus:border-amber-400" />
          </div>

          <div>
            <label className="block text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Mô tả</label>
            <textarea {...formik.getFieldProps("moTa")} rows="4" className="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2.5 text-white outline-none focus:border-amber-400" />
            {formik.touched.moTa && formik.errors.moTa && <p className="mt-1 text-xs text-rose-400">{formik.errors.moTa}</p>}
          </div>

          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Ngày khởi chiếu</label>
              <input type="date" {...formik.getFieldProps("ngayKhoiChieu")} className="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2.5 text-white outline-none focus:border-amber-400" />
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Đánh giá</label>
              <input type="number" min="0" max="10" {...formik.getFieldProps("danhGia")} className="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2.5 text-white outline-none focus:border-amber-400" />
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <label className="flex items-center gap-2 rounded-2xl border border-slate-800 bg-slate-900/70 px-4 py-3 text-sm text-slate-300">
              <input type="checkbox" {...formik.getFieldProps("dangChieu")} className="h-4 w-4 rounded border-slate-600 bg-slate-800" />
              Đang chiếu
            </label>
          </div>

          <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
            <button type="button" onClick={() => navigate("/admin/films")} className="rounded-2xl border border-slate-700 px-5 py-2.5 text-sm font-medium text-slate-300 transition hover:border-slate-500 hover:text-white">
              Hủy
            </button>
            <button type="submit" className="rounded-2xl bg-gradient-to-r from-amber-400 to-yellow-500 px-5 py-2.5 text-sm font-semibold text-slate-950 transition hover:brightness-110">
              Cập nhật phim
            </button>
          </div>
        </div>
